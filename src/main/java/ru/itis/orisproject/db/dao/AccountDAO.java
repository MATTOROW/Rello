...
INSERT INTO accounts (username, password, email, icon_path, session_id) VALUES (?, ?, ?, ?, ?)""";
    //language=sql
...
UPDATE accounts SET username = ?, password = ?, email = ?, icon_path = ?, session_id = ? WHERE username = ?""";
    ...
    public int save(Account acc) {
        try {
            ...
            preparedStatement.setString(5, acc.session_id());
    ...
    public int updateSessionByUsername(String username, String session_id) {
        try {
